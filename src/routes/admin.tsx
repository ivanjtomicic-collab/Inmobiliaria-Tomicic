  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      ...
      <form onSubmit={submit} className="grid gap-5 rounded-3xl border bg-card p-6 text-card-foreground shadow-sm md:grid-cols-2">
        ...
        <Field label="Tipo"><select value={form.type} onChange={(e) => update("type", e.target.value as PropertyInput["type"])}><option value="casa">Casa</option><option value="departamento">Departamento</option><option value="terreno">Lote</option><option value="campo">Campo</option><option value="local_comercial">Local comercial</option><option value="galpon">Galpón</option></select></Field>
        ...
        <Field label="Subir imagen"><input type="file" accept="image/jpeg,image/png,image/webp" disabled={uploading} onChange={(e) => void upload(e.target.files?.[0])} /></Field>
